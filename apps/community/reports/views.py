...
class PostReportView(APIView):
    ...
    def post(self, request, post_id: int):
        ...
        try:
            ...
            # create_post_report 내부에서 post_id 파싱/검증 실패 시
            return Response(
                {"code": "BAD_ID", "message": str(_e)},
                status=status.HTTP_400_BAD_REQUEST,
            )
        except DjangoValidationError as _e:
            msg = (
                getattr(_e, "message", None)
                or getattr(_e, "messages", [None])[0]
                or str(_e)
            )
            return Response(
            ...
            return Response(
                {"code": "CONFLICT", "message": "이미 처리된 요청입니다."},
